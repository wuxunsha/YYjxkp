...
<template>
  <div>
    ...
    <Row style="padding: 20px 15px; margin-top: 20px" class="area-container">
      ...
      <Col style="margin: 20px auto" v-if="btnShow(113101102107)">
        <Button type="primary" @click="indexinstruction(1, '')">提交</Button>
      </Col>
      <Col class="feedback-content" v-for="(item, index) in feedbackList" :key="'反馈与批示' + index">
        <div class="plan">
          <div class="info">
            <div>{{ item.content }}</div>
            <div class="btn-reply" @click="indexinstruction(2, item.id)" v-if="btnShow(113101102107)">回复</div>
          </div>
          <div class="desc">

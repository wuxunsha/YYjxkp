...
@import "./innovation-excellence-list.less";
.checkRule{
  ...
  <div class="layout">
        ...
        <Content class='from-content'>
          ...
          <Row class='create'>
            <Col span='24' class='create-template'>
              <Button type='primary' icon='ios-search' class='create-btn' style="margin-right:15px;" @click="newCreatedList" v-if="btnShow(114101101)">
                新增创新创优
              </Button>
          ...
          <Row class="table">
              <Table stripe   :columns='listHeader' :data='queryListData' disabled-hover>
                ...
                <template slot='operation' slot-scope="{ row }">
                  ...
                  <Icon  class="icon-font iconchakan" title="查看详情"  size="24" color="#0972E7" @click="seeDetailHandeler(row.id)" v-if="btnShow(114101104) && row.status === 1 || row.status === 2"/>
                  <!-- 修改 -->
                  <!-- v-if="row.status === 3 || row.status === 0"  -->
                  <Icon type="ios-create-outline" size="24" title="修改" color="#0972E7" @click="modifyHanderle(row)" v-if="btnShow(114101102) &&  row.status === 3 || row.status === 0"/>
                  <!-- 发布申请 -->
                  <!-- v-if="row.status === 0" -->
                  <Icon  class="ivu-icon icon-font icon212121" title="发布申请" size="24" color="#0972E7" @click="postApplication(row)" v-if="btnShow(114101105) && row.status === 0"/>
                  <!-- 删除 -->
                  <!-- v-if="row.status === 3 || row.status === 0"  -->
                  <Icon  class="ivu-icon icon-font iconshanchu" title="删除" size="24" color="red" @click="deleteHandeler(row.id)" v-if="btnShow(114101103) && row.status === 3 || row.status === 0" />
              </template>
              </Table>
          </Row>
          <!-- 分页 -->
    ...
    <!-- 提交批示 -->
        <Row class="performance-last-item">
          <Button type="primary" @click="submitApproval" v-if="btnShow(114101106)">提交批示</Button>
    ...
    <Modal v-model='addRuleStatus' width='40%' class='modal-information addRules' @on-visible-change="modalChange">
      ...
      <div class='modal-content'>
        ...
        <Row class="add-rule-item" style="margin-bottom:20px;">
           <Input  v-model="newRuleData.content" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入注释" style="width:85%;" />
        </Row>
        <Row class="add-rule-item" style="margin-bottom:20px;">
    ...
    <!-- 修改创新创优 -->
    <Modal v-model='modifyRulesStatus' width='60%' class='modal-information addTemplate' @on-visible-change="modalChange">
      <p class='modal-title'>
        <span>修改查看规则</span>
      </p>
      <div class='modal-content'>
        <Form ref='issueValidateRef'>
          <!-- 项目名称 -->
          <FormItem label='评级'>
            <Input  class='title-input' v-model="modifyRulesData.title"  maxlength="15" show-word-limit placeholder='请输入评级名称'/>
          </FormItem>
          <!-- 内容摘要 -->
          <FormItem label='计分'>
            <Input  class='title-input' v-model="modifyRulesData.score"  maxlength="25" show-word-limit placeholder='请输入计分'/>
          </FormItem>
          <!-- 项目说明 -->
          <FormItem label='注解'>
            <Input  type="textarea" v-model="modifyRulesData.content" maxlength="140" :rows="10" style="height:240px;" placeholder="请输入注解内容"/>
          </FormItem>
          <!-- 保存按钮 -->
          <Row class='modal-footer'>
            <Button class='add-btn' @click="saveModifyRulesHanderle">保 存</Button>
          </Row>
        </Form>
      </div>
    </Modal>
...
<script>
import { queryList, deleteList, ruleQueryList, saveRule, reply, save, ruleDeleteList, seeDetails, postApplication, modifyDepartment, updateRoules } from '@/api/Innovation-excellence'
...
export default {
  data () {
    return {
      ...
      fileNumberLimt: 3,
      // 文件格式限制
      fileFormat: ['jpeg', 'jpg', 'png', 'pdf', 'doc', 'docx'],
      // 修改查看规则的数据
      modifyRulesStatus: false,
      modifyRulesData: {
        content: '',
        id: '',
        score: '',
  ...
  methods: {
    ...
    uploadURL (uploadList) {
      ...
      uploadList.forEach(item => {
        item['downloadUrl'] = item.absolutePath
        item.fileType = ''
      })
      this.saveInfo.attachmentDTOS = uploadList
    },
    // 点击弹出修改规则
    modifyRules (row) {
      this.modifyRulesStatus = true
      this.modifyRulesData.content = row.content
      this.modifyRulesData.title = row.title
      this.modifyRulesData.id = row.id
      this.modifyRulesData.score = row.score
    },
    //  修改查看规则
    saveModifyRulesHanderle () {
      updateRoules(this.modifyRulesData).then(res => {
        if (res.data.code === 10000) {
          this.$Message.info('修改成功')
          this.modifyRulesStatus = false
          this.checkRule()
        }
      })
    },
    ...
    getNewSave (data) {
      save(data).then(res => {
        if (res.data.code === 10000) {
          ...
          this.getQueryList(this.queryLists)
          this.saveInfo.title = ''
          this.saveInfo.content = ''
          this.saveInfo.projectDesc = ''
          this.$refs.upload.emptyData()
        } else {
    ...
    checkRule () {
      ...
      ruleQueryList(this.queryLists).then(res => {
        if (res.data.code === 10000) {
    ...
    // 查询
    checkHandeler () {
      queryList(this.checkInfo).then(res => {
        if (res.data.code === 10000) {
          this.queryListData = res.data.data.dataList
          this.total = res.data.data.total
          this.pagesize = res.data.data.pagesize
    ...
    checkDetail () {
      seeDetails(this.seeDetailsId).then(res => {
        if (res.data.code === 10000) {
          ...
          console.log(res.data.data.attachmentEntityList.length)
          res.data.data.attachmentEntityList.forEach(item => {
            if (res.data.data.attachmentEntityList.length === 1) {
              this.defaultList3 = item.downloadUrl
            } else {
              this.defaultList3 += item.downloadUrl + '|'
            }
          })
    ...
    // 清空
    clearHandeler () {
      console.log(this.checkInfo.checkName)
    ...
    //  修改创新创优
    saveModifyHanderle () {
      modifyDepartment(this.modifyList).then(res => {
        if (res.data.code === 10000) {
          this.$Message.info('修改成功')

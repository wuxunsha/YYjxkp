            ...
            <Content class='from-content'>
                ...
                <Row class="content">
                    ...
                    <Form :model="assessData">
                        <!-- 考核机构 -->
                        <FormItem class="assess-org">
                            <Row>
                                <Col span="3" class="assess-org-label">
                                    <span>此次考核机构</span>
                                </Col>
                                <Col span="9">
                                    <Select v-model="perOrg" multiple>
                                        <Option v-for="item in orgList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                    </Select>
                                </Col>
                                <Col span="3" class="assess-org-reminder">
                                    <span>（可多选）</span>
                                </Col>
                            </Row>
                        </FormItem>
                        <!-- 日常考核时间 -->
                        <FormItem class="assess-time">
                            <Row>
                                <Col span="3" class="assess-org-label">
                                    <span>考核时间</span>
                                </Col>
                                <Col span="4">
                                    <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="开始时间" @on-change="startTimeChange" transfer></DatePicker>
                                </Col>
                                <Col span="1" >
                                <span>~</span>
                                </Col>
                                <Col span="4">
                                    <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="截止时间" @on-change="endTimeChange" transfer></DatePicker>
                                </Col>
                            </Row>
                        </FormItem>
                        <!-- 时间节点设置 -->
                        ...
                        <FormItem class="time-set">
                            <Row>
                                <Col span="8">
                                    <Row>
                                        <Col span="9" style="text-align: right;padding-right:10px;">
                                            <span>数&nbsp;&nbsp;&nbsp;据&nbsp;&nbsp;&nbsp;&nbsp;采&nbsp;&nbsp;&nbsp;&nbsp;集</span>
                                        </Col>
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据采集截止时间" @on-change="gatherEndTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据采集截止时间" @on-change="gatherTimeoutTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                            ...
                            <Row>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="reCheckTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="reCheckTimeoutTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="2" style="text-align:center;">
                            ...
                            <Row>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="editTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="editTimeoutTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="2" style="text-align:center;">
                            ...
                            <Row>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="issueTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="8">
                                    <Row>
                                        ...
                                        <Col span="15">
                                            <DatePicker  type="datetime" format="yyyy-MM-dd HH" placeholder="请选择数据截止时间" @on-change="issueTimeoutTimeChange" transfer></DatePicker>
                                        </Col>
                                    </Row>
                                </Col>
                                <Col span="2" style="text-align:center;">
...
export default {
  ...
  methods: {
    ...
    startTimeChange (time) {
      ...
      this.assessData.perStartTime = timeStamp
    },
    // 考核截止时间改变
    endTimeChange (time) {
      let timeStamp = getTimeStamp(time + ':00')
      this.assessData.perEndTime = timeStamp
    },
    // 数据采集截止时间改变
    ...
    initiateAssess () {
      ...
      this.assessData.perOrgIds = this.perOrg.toString()
      if (!this.assessData.perOrgIds) {
        this.$Message.error('请选择此次考核机构')
      } else if (!this.assessData.perStartTime) {
        this.$Message.error('请选择此次考核开始时间')
      } else if (!this.assessData.perEndTime) {
        this.$Message.error('请选择此次考核截止时间')
      } else if (!this.assessData.dataCEndTime) {
      ...
      } else if (!this.assessData.perCodeList) {
        this.$Message.error('请选择考核项目')
      } else if (this.assessData.perEndTime >= this.presentTime && this.assessData.perStartTime >= this.presentTime) {
        this.$Message.error('考核开始或截止时间不能大于当前时间')
      } else if (this.assessData.perStartTime >= this.assessData.perEndTime) {
        this.$Message.error('考核开始时间不能大于或等于截止时间')
      } else if (this.assessData.dataCEndTime <= this.presentTime) {
        this.$Message.error('数据采集截止时间要大于当前时间')
      } else if (this.assessData.dataCEndTime <= this.assessData.perEndTime) {
        this.$Message.error('数据采集截止时间要大于考核截止时间')

    ...
    <div class="container">
        <div class="content">
            <div class="header">
                ...
                <div class="steps">
                    <article-steps :currentId="4" :stepsList="stepsList"></article-steps>
                </div>
                        <div class="tabs">
          <Tabs value="name1">
          <TabPane label="考核结果" name="name1">
            <Row class="modify-submit">
                <Button type="primary" @click="submitDataPerformance" v-if="btnShow(112101105102)" >提交</Button>
            </Row>
            <Row>
          ...
          <TabPane label="反馈与批示" name="name2">
            ...
            <Row class="feedback-content-item" v-for="item in feedbackList" :key="item.id">
                ...
                <div v-for="item1 in item.children" :key="item1.id" style="margin-top:10px;">
...
<script>
import { getDataCheckList, submitPerformance, processSubmission } from '@/api/performance-appraisals'
...
export default {
  ...
  methods: {
    ...
    ok () {
      ...
      submitPerformance(data).then(res => {
        if (res.data.code === 10000) {
    ...
    operation () {
      return (h, params) => {
        ...
        return h('div', [this.btnShow(112101105101) && detailTooltip])
      }
    },
    detailData (params) {
      console.log(params)

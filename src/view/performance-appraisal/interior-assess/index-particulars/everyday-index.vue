      ...
      <TabPane icon="ios-trending-up" tab="OrgTabs" :label="item.indexTypeName" :name="JSON.stringify(item)" :key="index">
        <Row class="header">
          ...
          <Col class="quota-table">
            <Row>
              <Col span="24">
                <Button type="primary" @click="initiateAssess" v-if="btnShow(112102102101)">发起考核</Button>
              </Col>
            </Row>
          </Col>
          <Col class="quota-table">
...
<script>
import {
  getloginperfomancebatchindextypepeer,
  getindexbyyears,
...
export default {
  ...
  created () {
    // 获取上级待办指标类型
    this.getloginperfomancebatchindextypepeerType()
  },
  methods: {
    operation () {
      return (h, params) => {
    ...
    // 获取当前登录用户本级指标类型
    getloginperfomancebatchindextypepeerType () {
      getloginperfomancebatchindextypepeer().then(res => {

...
<template>
  <div>
    ...
    <Row class="content">
      <Col class="btn-add">
        <Button type="primary" class="add" @click="showInformationModal" v-if="btnShow(115101101)">
      ...
      <Col>
        <Table :row-class-name="rowClassName" :columns="columns" :data="informationData" :border="false">
          ...
          <template slot='operation' slot-scope="{ row }">
            <Icon class="icon-font iconcaozuo-bianji" size="24" color="#0972E7" @click="editModal(row.id)" v-if="btnShow(115101102) && row.status === 1 || row.status === 3 || row.status === 5 || row.status === 6" title="编辑"/>
            <Icon class="icon-font iconchakanyanjingshishifenxi" size="24" color="#0972E7" @click="examine(row.id)" v-if="btnShow(115101103) && row.status != 1" title="查看详情" />
            <Icon class="icon-font icondaishenhe" size="24" color="#0972E7" @click="submitAudit(row.id)" v-if="btnShow(115101108) && row.status === 1" title="提交"/>
            <Icon class="icon-font iconxiaxiansvg online" size="24" color="#0972E7" title="上线" @click="online(row.id)" v-if="btnShow(115101106) && row.status === 3 || row.status === 5"/>
            <Icon class="icon-font iconxiaxiansvg" size="24" color="#0972E7" title="下线" @click="offlinet(row.id)" v-if="btnShow(115101107) && row.status === 4"/>
          </template>
        </Table>
      </Col>
      <Col class="page">
...
export default {
  ...
  methods: {
    // 给富文本设置默认提示语
    // setPlaceholder () {
    //   this.$refs.addEditor.setHtml('请输入通告内容')
    // },
    // 获取页签ID
    getTabId () {
      getlistbytype(7).then(res => {
        if (res.data.code === 10000) {
          this.tab = res.data.data[1].nameValue
          // 获取绩效资讯全部数据

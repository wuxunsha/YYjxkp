          ...
          <Row class="peoples">
            <div class="achievements">
              ...
              <Row class="new-question">
                <Row>
                    <Col span="3">
                      <span @click="addIssue">新增选题</span>
                    </Col>
                    <Col span="3">
                      <span @click="addCustomIssue">新增自定义问题</span>
                    </Col>
                </Row>
              </Row>
            </div>
  ...
  methods: {
    ...
    getFindPortalSurvey () {
      findPortalSurvey(this.$route.query.id).then(res => {
        if (res.data.code === 10000) {
          let radio = res.data.data.realNameStatus + ''
          res.data.data.realNameStatus = radio
    ...
    quesSubmit () {
      this.$refs.formValidateRef.validate((valid) => {
        if (valid) {
          updatePortalSurvey(this.surveyViewObj).then(res => {
            if (res.data.code === 10000) {

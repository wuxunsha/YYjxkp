...
<div class="layout">
  <Layout>
    ...
    <Content class="content">
      ...
      <!-- 调查与方向 -->
      <Tabs :animated="false" class="tabs" value="name1">
        <TabPane label="问卷调查题" name="name1" class="tabs1">
          <Row v-for="(item, index) in surveyViewObj.surveySubtitleEntityList" v-bind:key="index">
            <Row class="survey-type">
              <Col span="24">{{mapSequence[index]}}、{{item.title}}({{item.score}}分)</Col>
            </Row>
            <List :split="false" class="list">
              <ListItem v-for="(item1, index1) in item.questtionEntities" v-bind:key="index1">
                <Row style="width: 100%;">
                  <Row style="width: 100%;">
                    <Col span="24" class="problem">
                  ...
                  <Row class="option" v-for="(item2, index2) in item1.quesList" v-bind:key="item2.id">
                    <span>
                      <span>{{mapOption[index2]}}、</span>
                      <span>{{item1.quesList[index2]}}</span>
                      <span class="people-counting" >{{item1.quesScoreList[index2] ? item1.quesScoreList[index2] : '0'}}分</span>
                    </span>
                  </Row>
                </Row>
              </ListItem>

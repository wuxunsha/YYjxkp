        ...
        <Col>
          <Table :row-class-name="rowClassName" :columns="columns" :data="socieEvalQuesList" :border="false">
            <template slot="operation" slot-scope="{row}">
              ...
              <!-- 查看 -->
              <Icon class="icon-font iconchakanyanjingshishifenxi" size="24" color="#0972E7" @click="questionnaire(row.surveyId)" v-if="btnShow(116102102104) && row.status != 1" title="查看详情" />
              <!-- 提交 -->
...
export default {
  ...
  methods: {
    ...
    questionnaire (id) {
      this.$router.push({
        path: 'superior_examine_particulars', query: { id: id }
      })
    },
    // 提交问卷
    ...
    editQuestionnaire (id) {
      this.$router.push({
        path: 'superior_entering_content', query: { id: id }
      })
    }
  }
}

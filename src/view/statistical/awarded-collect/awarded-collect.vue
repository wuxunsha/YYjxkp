          ...
          <Row>
            <Table :row-class-name="rowClassName" :columns='columns' :data="PySumInnoList" :border="false">
              <template slot='operation' slot-scope="{ row }">
                <!-- 查看 -->
                <Icon class="icon-font iconchakanyanjingshishifenxi" size="24" color="#0972E7" title="查看详情" @click="examineDraft(row.id)" v-if="btnShow(123103102)"></Icon>
               </template>
            </Table>
          </Row>
          <Row class='page'>
...
export default {
  ...
  methods: {
    ...
    examineDraft (id) {
      this.$router.push({
        path: 'examine_awarded_collect', query: { id: id }
      })
    }
  }
}

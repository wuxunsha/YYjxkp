          ...
          <Row>
            <Table :row-class-name="rowClassName" :columns='columns' :data="pySumList" :border="false">
              <template slot='operation' slot-scope="{ row }">
                <!-- 查看 -->
                <Icon class="icon-font iconchakanyanjingshishifenxi" size="24" color="#0972E7" title="查看详情" @click="examineDraft(row.id)" v-if="btnShow(123105103)"></Icon>
               </template>
            </Table>
          </Row>
          <Row class='page'>
...
export default {
  ...
  methods: {
    ...
    examineDraft (id) {
      this.$router.push({
        path: 'examine_scoring_particulars', query: { id: id }
      })
    }
  }
}
